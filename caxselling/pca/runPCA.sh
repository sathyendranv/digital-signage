...
check() {
    ...
    if test -e ./docker/servers.json; then
        ...
        if [ "$lines" -ge 11 ]; then
            mess_ok2 "\tPGAdmin Server.json file: " "Found"
        else
            mess_er2 "\tPGAdmin Server.json file: " "Not Found"
            exit 1
        fi
    else
        mess_er2 "\tPGAdmin Server.json file: " "Not Found"
...
start() {
    ##Starting Process
    mess_inf "Starting PCA containers ..."
...
stop() {
    ##Stopping Process
    mess_inf "Stopping PCA containers ..."
...
help() {
    mess_inf "Usage: "
    mess_op1 "\t./runPCA.sh [start | stop | check | help]"
    mess_op1 "\nOptions:"
    mess_op2 "\tstart: " "Start the PCA containers."
    mess_op2 "\tstop: " "Stop the PCA containers."
